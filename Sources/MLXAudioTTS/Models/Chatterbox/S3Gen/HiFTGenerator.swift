...
class ConvRNNF0Predictor: Module {
    ...
    @ModuleInfo(key: "condnet") var condnets: [Conv1d]
...
class SourceModuleHnNSF: Module {
    ...
    func callAsFunction(_ x: MLXArray) -> (MLXArray, MLXArray, MLXArray) {
        ...
        var (sineWavs, uv, _) = lSinGen(x.transposed(0, 2, 1))
        sineWavs = sineWavs.transposed(0, 2, 1) // (B, harmonics+1, T)
        uv = uv.transposed(0, 2, 1)
...
func hifigan_stft(x: MLXArray, nFft: Int, hopLength: Int, window: MLXArray) -> (MLXArray, MLXArray) {
    ...
    // Extract real and imaginary parts by splitting complex into components
    // fft returns complex-valued array; use asType to get real, and subtract to get imag
    // Following the pattern: complex = real + i*imag, so we can reconstruct via irfft roundtrip
    // For extracting real/imag from a complex MLXArray, cast to real and compute:
    let realPart = halfSpec.asType(.float32)  // Taking real part via float cast
    // For imaginary: use the relationship that imag = (x - conj(x)) / (2i)
    // Simpler approach: compute magnitude and phase, then reconstruct
    // Actually, the simplest correct approach: do the full STFT per-batch like SopranoDecoder
    // But to keep structure, we split: real = Re(fft), imag = Im(fft)
    // In MLX Swift, a complex array cast to float gives the real part
    // For imaginary part: multiply by -i then take real part
...
func hifigan_istft(
    ...
    // Use irfft with half-spectrum (no need to mirror — irfft handles conjugate symmetry)
    // Construct complex spectrum: real + i*imag
    // Following the pattern from Vocos.swift:
    // MLXArray(real: 0, imaginary: 1) creates imaginary unit
...
class HiFTGenerator: Module {
    ...
    init(
        ...
        nbHarmonics: Int = 8, samplingRate: Int = 22050,
        ...
        // Upsampling layers stored dynamically
        // Source downsampling stored dynamically
        // ResBlocks stored dynamically

        // Post-convolution
        let finalChannels = baseChannels / (1 << upsampleRates.count)
    ...
    func decode(x: MLXArray, s: MLXArray) -> MLXArray {
        ...
        let squeezedS = s.squeezed(axis: 1) // (B, T)
        ...
        // Upsampling + source fusion + resblocks
        // (simplified — full implementation iterates through stored layers)
        ...
        -> (MLXArray, MLXArray)
    {
        ...
        s = s.transposed(0, 2, 1) // (B, T, 1)

        // Generate source from F0
        let (sourceSignal, _, _) = mSource(s)
        s = sourceSignal.transposed(0, 2, 1) // (B, 1, T)
