...
#           .build/arm64-apple-macosx/{config}/MLXAudioPackageTests.xctest/Contents/MacOS/mlx.metallib
#
# Mirrors mlx-swift-lm/scripts/build-metallib.sh — same staging layout so a
# single shell driver can warm the metallib for either repo.
...
build_metallib_for_config() {
    ...
    local XCTEST_MACOS="$BUILD_DIR/MLXAudioPackageTests.xctest/Contents/MacOS"
...
echo "  ./scripts/benchmark.sh --pipeline <pipeline> --model <model>"
